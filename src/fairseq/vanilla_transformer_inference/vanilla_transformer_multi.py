...
@register_model("vanilla_transformer_multi")
...
class LinearTransformerMultiHeadDecoder(FairseqDecoder):
    def __init__(self, args, task):
        ...
        layer = nn.TransformerEncoderLayer(
            ...
            dim_feedforward=4*args.embed_dim,
            dropout=0.1,
            batch_first=True
        )
        ...
        self.attn_mask = torch.triu(torch.ones(self.max_pos, self.max_pos), diagonal=1).bool()
    ...
    def extract_features(
        ...
        len_mask = x[..., 0].eq(self.pad_idx).long().to(x.device)
        

        if self.perm_inv > 1:
            rel_pos = pad_mask * x[..., 4]
            measure_ids = pad_mask * x[..., 5]
            pos_emb = self.wRpe(rel_pos) + self.wMpe(measure_ids)
        ...
        attn_mask = (self.attn_mask[:seq_len, :seq_len]).to(x.device)
        outputs = self.model(x, attn_mask, len_mask)
        outputs = self.ln_f(outputs)
        # shape is (bsz, seq_len, embed_dim)
        outputs = outputs[:, -1, :] # take last
        # become (bsz, embed_dim)
        
        return outputs, None
    

@register_model_architecture("vanilla_transformer_multi", "vanilla_transformer_multi")
...
@register_model_architecture("vanilla_transformer_multi", "vanilla_transformer_multi_large")
def base_architecture(args):
